...
if ~isempty(X)
    ...
    for i=1:num_matrices
        matrix = matrices(i);
        matrices_val(i) = {matrix{1,1}(validation_set,:)};
        matrices_ana(i) = {matrix{1,1}(analysis_set,:)};
    end
...
while count_ns<input.coun_ts_limit
    ...
    if any(ff==input.grid_dynamic.onset)
        ...
        if ~isempty(X)
            ...
            temp_density = 1;
            for i=1:num_matrices
                temp_density = grid(i).density*temp_density;
            end
        ...
        if ~isempty(X)
            ...
            [weigths_combination, hyperopt_sets] = cv_weights_ext(matrices_ana, grid, size_sets_hyperopt);
            weights_ICV_collection = cell(size(weigths_combination,1),1);
        end
    end
        ...
        for ob=1:OB
            ...
            if ~isempty(X)
                ...
                save([hyperopt_folder '/hyperopt_partition.mat'],...
                    ...
                    'train_DiagNames', 'test_DiagNames',...
                    'cv_inner_TrainInd', 'cv_inner_TestInd','correlation_method',...
                    'weigths_combination', 'size_sets_hyperopt', 'correction_target', '-v7.3');
        ...
        switch selection_train % 1=within one OCV fold, 2=across all OCV folds
            case 1 % within one OCV fold
                ...
                if ~isempty(X)
                    ...
                    weights_opt = weigths_combination(I_max,:);
                end
                ...
                for ob=1:OB
                    ...
                    if ~isempty(X)
                        ...
                        for i=1:num_matrices
                            IN_matrices(i).train = matrices_ana{i}(output.CV.cv_outer_indices.TrainInd{ob,w},:);
                            IN_matrices(i).test = matrices_ana{i}(output.CV.cv_outer_indices.TestInd{ob,w},:);
                        end
                    end
                    ...
                    switch selection_retrain
                        ...
                        case 1 % retrain on entirety of folds
                            if ~isempty(X)
                                ...
                                [RHO_opt_collection(1,ob), weights_opt_collection(:,ob), lVs_opt_temp] = cv_gspls_full(OUT_matrices, weights_opt, correlation_method);
                            end
                            ...
                            if weights_log
                                if ~isempty(X)
                                    ...
                                    for i=1:num_matrices
                                        weights{i} = RHO_opt_collection(1,ob)*ones(size(weights_opt_collection(:,ob),1),1)';
                                    end
                                    weights_V_1 = RHO_opt_collection(1,ob)*ones(size(V_opt_collection_temp));
                                    weights_V = cat(3, weights_V, weights_V_1);

                                    save([hyperopt_folder, '/weights_check.mat'], 'weights', 'weights_V');
                                    lVs_opt_collection = cat(1, lVs_opt_collection, lVs_opt_temp);
                        ...
                        case 2 % use already existing u and v from inner folds without retraining
                            ...
                            if ~isempty(X)
                                ...
                                lVs_opt_collection = cat(1, epsilon_opt_collection, epsilon_opt_temp);
                            end
                ...
                switch input.merge_retrain
                    ...
                    otherwise
                        ...
                        if ~isempty(X)
                            ...
                            for i= 1:num_matrices
                                weights_opt{i} = dp_trainmerge_single(weights_opt_collection{i}, input.merge_retrain, 2, weights{i});
                            end
                            save([hyperopt_folder '/opt_vars_check.mat'],...
                                'RHO_opt_collection','weights_opt_collection', 'RHO_opt', 'weights_opt', 'RHO_ICV_collection', '-v7.3');
