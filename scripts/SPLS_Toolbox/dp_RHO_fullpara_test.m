...
function [RHO_output, success] = dp_RHO_fullpara_test(spls_standalone_path, analysis_folder, type_analysis, mem_total, max_sim_jobs, queue_name, total_jobs, size_sets) 
            ...
            for i=1:total_jobs
                if ~exist([analysis_folder '/init_' num2str(i) '.txt'])
                    switch type_analysis
                        case 'hyperopt'
                            dp_RHO_avg_100_sets(num2str(i), num2str(size_sets), analysis_folder);
                        case 'permutation'
