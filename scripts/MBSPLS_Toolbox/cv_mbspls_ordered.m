function [weights, covariances, Vs, success, k, diff, dims] = cv_mbspls_ordered(matrices, cs, gs, e, itr_lim, printflag, Vs_original)
%
...
for i = 1:num_matrices
    ...
    for j = i+1:num_matrices
        ...
        u_weights_pairs{i,j}(:,1) = Us{i,j}(:,1);
        u_weights_pairs{i,j}(:,1) = u_weights_pairs{i,j}(:,1)./norm(u_weights_pairs{i,j}(:,1)); % normalise

        v_weights_pairs{i,j}(:,1) = Vs{i,j}(:,1);
        v_weights_pairs{i,j}(:,1) = v_weights_pairs{i,j}(:,1)./norm(v_weights_pairs{i,j}(:,1)); % normalise
...
for i = 1:num_matrices
    ...
    for j = i+1:num_matrices
        mbspls_pair{i,j}.us_combined = zeros(num_weights(i), 1);
        mbspls_pair{i,j}.vs_combined = zeros(num_weights(j), 1);
    end
end

for i = 1:num_matrices
    weights_combined = zeros(num_weights(i),1);
    %     switch i
    %         case i == 1
    %             for j = i+1:num_matrices
    %                 weights_combined = weights_combined + gs(i,j) * u_weights_pairs{i,j}(:,1);
    %             end
    %
    %         case i > 1
    for j = 1:i-1
        weights_combined = weights_combined + gs(j,i) * v_weights_pairs{j,i}(:,1);
    end
    for j = i+1:num_matrices
        weights_combined = weights_combined + gs(i,j) * u_weights_pairs{i,j}(:,1);
    end
    % end
    weights{i}(:, 1) = weights_combined;
    weights{i}(:, 1) = weights{i}(:, 1) ./ norm(weights{i}(:, 1)); 
end
...
while diff > e && success
    ...
    for i = 1:num_matrices
        ...
        for j = 1:i-1
            weights_temp(:,j) = covariances{j,i}' * weights{j}(:, 2); 
        end
        for j = i+1:num_matrices
            weights_temp(:,j) = covariances{i,j} * weights{j}(:, 1);
        end
