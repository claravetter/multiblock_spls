...
function cv_mbspls_ICV_bootstrap_csv(i, analysis_folder)
...
train_data_matrices = m_data.train_data_matrices;
...
for ii=1:size(bootsam,2)
        ...
        for num_m=1:length(train_data_matrices)
            matrices_boot{num_m} = train_data_matrices{num_m}(bootsam(:,ii),:);
            covars_boot{num_m} = covars{num_m}(bootsam(:,ii),:);
        end
    ...
    for num_m= 1:size(train_data_matrices,2)
        cs_method_bootstrap{num_m}.subgroup_train = matches(labels_boot, cs_method_bootstrap{num_m}.correction_subgroup);
        cs_method_bootstrap{num_m}.subgroup_test = matches(labels_boot, cs_method_bootstrap{num_m}.correction_subgroup);
    end
