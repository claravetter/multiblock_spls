...
analysis_folder = fullfile(setup.analysis_folder, analysis_name);
if ~exist(analysis_folder,'dir')
    ...
    % Loop until a unique folder is found and created
    while true
        analysis_name = [input.name '_' num2str(nn)];
        input.name = analysis_name; 
        analysis_folder = fullfile(setup.analysis_folder, analysis_name);
...
system(['cp ', datafile, ' analysis_folder/']);
