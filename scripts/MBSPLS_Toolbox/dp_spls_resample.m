%% DP resample SPLS function including Procrustes transformation

function [u, v, success] = dp_spls_resample(X, Y, cu, cv, V_original)
%
...
%--- compute SVD
[U_resampled,S_resampled,V_resampled] = svd(C,0);

%--- Perform Procrustes Rotation if permutation flag is set
C_temp = V_original'*V_resampled;
[N,~,P] = svd(C_temp,0);
Q = N*P';
V = V_resampled * S_resampled * Q;
U = U_resampled * S_resampled * Q;
