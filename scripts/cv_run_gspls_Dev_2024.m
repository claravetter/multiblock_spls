...
while count_ns<input.coun_ts_limit
    ...
    for w=1:W
        ...
        switch selection_train % 1=within one OCV fold, 2=across all OCV folds
            case 1 % within one OCV fold
                    ...
                    switch selection_retrain
                        ...
                        case 1 % retrain on entirety of folds
                            if ~isempty(X)
                                ...
                                [RHO_opt_collection(1,ob), weights_opt_collection(:,ob), Vs_opt_temp] = cv_gspls_full(OUT_matrices.train, OUT_matrices.test, c_weights_opt, correlation_method);;
                                Vs_opt_collection = cat(3, Vs_opt_collection, Vs_opt_temp);
                            end
                    ...
                    otherwise
                        ...
                        if ~isempty(X)
                            ...
                            for num_m= 1:num_matrices
                                ...
                                Vs_opt = dp_trainmerge_single(Vs_opt_collection{num_m}, input.merge_retrain, 3, weights_Vs);
                            end
                            save([hyperopt_folder '/opt_vars_check.mat'],...
                ...
                RHO_b_collection = cv_ICV_main_csv_mult_sequential(permutation_folder, 'permutation', 1, setup.cache_path, num_matrices, xy_spls_flag);
    ...
    [RHO_boot, weights_boot] = cv_ICV_main_csv_mult_sequential(bootstrap_folder, 'bootstrap', 1, setup.cache_path, num_matrices, xy_spls_flag);
    if xy_spls_flag
        ...
        switch input.validation_train
            case 1 % retrain
                ...
                if ~isempty(X)
                    ...
                    RHO_b_collection = cv_ICV_main_csv_mult_sequential(permutation_folder, 'permutation', 1, setup.cache_path, num_matrices);
