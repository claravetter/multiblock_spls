...
setup.analysis_folder_hpc       = ['/volume/projects/CV_gs_PLS/Analysis/PLEXUS/', setup.date]; % analysis folder is created automatically using the current date
...
input.X = matrices{3}; % 3 has to be first because it's bigger
input.Y = matrices{2};
...
input.density               = 20; % should this be the same for all matrices or per matrix? 
...
input.name = ['CV_spls_2matrices_matrices23_', num2str(input.outer_folds), 'x', num2str(input.inner_folds), '_', num2str(input.permutation_testing), 'perm_', num2str(input.bootstrap_testing), 'boot_', num2str(input.density), 'density']; 
...
input.datafile_hpc = [setup.analysis_folder_hpc, '/' setup.date, '_', input.name, '_datafile.mat']; % Path for storing datafile containing input and setup

mkdir(setup.analysis_folder_hpc); % Creates analysis folder
save([input.datafile_hpc], 'setup', 'input'); % Saves datafile in analysis folder
...
% 
%dp_bash_main_job_slim_addedruntime_mult(input.datafile);
