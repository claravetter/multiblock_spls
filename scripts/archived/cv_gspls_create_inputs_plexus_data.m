...
load("/volume/projects/CV_gs_PLS/Data/mri_plexus_clinical_matrices.mat")
coreflag= 1;
%matrices = {X,Y,Z};
...
setup.mem_request           = 10; % Memory request for master and slave jobs, in GB, maybe decrease to 2 or 3
...
if coreflag
    ...
    switch setup.matlab_version 
        case 'R2022a'
            setup.matlab_path       = '/data/core-psy-pronia/opt/matlab/v912';
        case 'R2022b'
            setup.matlab_path       = '/data/core-psy-pronia/opt/matlab/v913';
    end
    setup.spls_standalone_path  = '/data/core-psy-archive/projects/CV_mbspls/mbspls/cv_gspls_standalone_SLURM_Dev_2024'; % Path of the SPLS Toolboxsetup.date                  = date; % automatic date
    ...
    setup.analysis_folder_hpc = ['/volume/projects/CV_gs_PLS/Analysis/PLEXUS/', setup.date];
    setup.partition             = 'jobs-matlab';
    setup.nodes                 = 1; 
    setup.account               = 'core-psy';
    setup.user                  = 'clvetter';
end
...
if size(input.matrices,2) >2
    input.matrix_norm           = 2; % Define which matrix norm is used to compute association between latent scores of matrices (used for significance testing of LV): default = 2 if more than 2 matrices, otherwise 0 (in this case, correlation coefficient as defined above is used) 
else
    input.matrix_norm           = 0; % Define which matrix norm is used to compute association between latent scores of matrices (used for significance testing of LV): default = 2 if more than 2 matrices, otherwise 0 (in this case, correlation coefficient as defined above is used) 
end
%if exist(CV)
...
if ~coreflag
    input.name = ['CV_gspls_plexus_mri_clinical_', num2str(input.outer_folds), 'x', num2str(input.inner_folds), '_', num2str(input.permutation_testing), 'perm_', num2str(input.bootstrap_testing), 'boot_', num2str(input.density{1}), 'density']; 
    ...
    cv_gspls_bash_main_job_slim_addedruntime_mult(input.datafile);
else 
  input.name = ['CV_gspls_plexus_mri_clinical_', num2str(input.outer_folds), 'x', num2str(input.inner_folds), '_', num2str(input.permutation_testing), 'perm_', num2str(input.bootstrap_testing), 'boot_', num2str(input.density{1}), 'density']; 

    input.datafile = [setup.analysis_folder_hpc, '/' setup.date, '_', input.name, '_datafile.mat']; % Path for storing datafile containing input and setup

    mkdir(setup.analysis_folder_hpc); % Creates analysis folder
    save([input.datafile], 'setup', 'input'); % Saves datafile in analysis folder
