...
function [explained_covariance_per_component, total_explained_covariance] = cv_compute_explained_covariance_LV(matrices, T_matrices)
    ...
    n_LVs = size(T_matrices{1}, 2);  % Number of latent components (assuming T1 has the components)

    % Initialize a matrix to store correlation-based explained covariance
    explained_covariance = zeros(n_LVs, n_Xs, n_Xs);

    % Loop over latent components to compute explained covariance using correlation
    for h = 1:n_LVs
        for i = 1:n_Xs
            for j = 1:n_Xs
                if i ~= j  % Only between different blocks
                    ...
                    % Compute the Pearson correlation between T_i and T_j
                    correlation_Ti_Tj = corr(T_i, T_j);  % Scalar correlation

                    % Assign the correlation as the explained covariance for this component
                    explained_covariance(h, i, j) = abs(correlation_Ti_Tj);  % Absolute value for consistency
                end
            end
        end
    end

    % Calculate average explained correlation across block pairs for each latent component
        ...
        for i = 1:n_Xs
            for j = 1:n_Xs
                if i ~= j  % Avoid self-correlation
                    total_corr = total_corr + explained_covariance(h, i, j);
                    count = count + 1;
                end
            end
        end
        explained_covariance_per_component(h) = total_corr / count;  % Average explained correlation
    end

    % Calculate the total explained covariance across all components
    total_explained_covariance = sum(explained_covariance_per_component) / n_LVs;

    % Display results
    disp('Normalized explained covariance per latent component (correlation-based):');
    disp(explained_covariance_per_component);
    disp(['Total explained covariance (as proportion of total shared correlation): ', num2str(total_explained_covariance)]);
end
