...
compiled_mbspls_name = ['MBSPLS_DEV_Jan2025_' matlab_version];
compiled_mbspls_version = 'v2';
...
user_note = 'imputation of input matrices (SeqkNN); projection deflation with NaN values in matrix'; % change
...
%% 
github = 0; 
github_directory = '/opt/PrecisionCodeRep/SPLS_Toolbox/mbSPLS/2_Analysis/';
...
%% GitHub
if github 
    system(['cp -r ', fullfile(compiled_mbspls_directory, compiled_mbspls_name) , ' ', github_directory]);
    disp(['cd ', github_directory]);
    disp('git add .');
    disp(['git commit -m ''', user_note, '''']);
    disp('git push');
end
